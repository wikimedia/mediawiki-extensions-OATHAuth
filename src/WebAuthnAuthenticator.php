...
class WebAuthnAuthenticator {
	...
	private function startAuthenticationInternal( ?OATHUser $user, bool $userVerificationRequired ): Status {
		if ( $user ) {
			...
			if ( !$canAuthenticate->isGood() ) {
				$this->logger->info(
					"User {user} cannot authenticate using WebAuthn", [
	...
	public function continueAuthentication(
		...
		if ( !$canAuthenticate->isGood() ) {
			$this->logger->warning(
				"User {user} lost ability to authenticate using WebAuthn mid-request", [
					'user' => $user->getUser()->getName(),
				]
			);
			return $canAuthenticate;
	...
	public function startRegistration( OATHUser $user, bool $passkeyMode = false ): Status {
		...
		if ( !$canRegister->isGood() ) {
			$this->logger->info(
				"User {user} cannot register a WebAuthn credential"
			);
			return $canRegister;
		}
	...
	public function continueRegistration(
		...
		if ( !$canRegister->isGood() ) {
			$this->logger->warning(
				"User {user} lost ability to register using WebAuthn ability mid-request",
				[
					'user' => $user->getUser()->getName(),
				]
			);
